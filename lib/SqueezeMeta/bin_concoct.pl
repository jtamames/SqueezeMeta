...
foreach my $sam(@samfiles) {
	...
	$bamfile=~s/sam$/bam/;
	$command="$samtools_soft view -b -\@$numthreads -S $samfile -o $bamfile > /dev/null 2>&1";
	print outsyslog "  Creating BAM for $sam: $command\n";
	print "  Creating BAM for $sam\n";
	system $command;
	my $sorted=$bamfile;
	$sorted=~s/\.bam$/\.sorted/;
	$command="$samtools_soft sort $bamfile $sorted -\@$numthreads > /dev/null 2>&1";
	print "  Sorting BAM\n";
	print outsyslog "  Sorting BAM: $command\n";
	system $command;
	$command="$samtools_soft index $sorted.bam > /dev/null 2>&1";
