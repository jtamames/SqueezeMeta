...
plotFunctions = function(SQM, fun_level = 'KEGG', count = 'tpm', N = 25, fun = c(), ignore_unclassified = T, gradient_col = c('ghostwhite', 'dodgerblue4'), base_size = 11)
    ...
    if(!count %in% c('abund', 'bases', 'tpm', 'copy_number'))
        {
        stop('count must be "abund", "bases", "tpm" or "copy_number"')
        }
    if( count == 'bases' & is.null(SQM[['functions']][[fun_level]][[count]]) )
        {
        stop('There are no tables containing aggregated bases per function in your project, possibly because it was it was generated by sqm-reads.pl')
        }
    if( count == 'copy_number' & is.null(SQM[['functions']][[fun_level]][[count]]) )
        {
        stop('There are no copy number tables in your project, possibly because RecA was not present in the metagenome or it was generated by sqm-reads.pl')
        }
    if (is.null(fun) & N <= 0)
        {
    ...
    nice_label = c(abund='Raw abundance', bases='Bases', tpm='TPM', copy_number='Copy number')[count]
