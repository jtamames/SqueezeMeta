...
aggregate.taxa = function(SQM, rank, tax_source)
    ...
    if(!class(SQM)=='SQM') { stop('The first argument must be a SQM object')}
    res = aggregate(data.table(SQM[[tax_source]][['abund']]), by=list(SQM[[tax_source]][['tax']][,rank]), FUN=sum)
...
aggregate.fun = function(SQM, fun, trusted_functions_only, ignore_unclassified_functions)
    ...
    abund                      = aggregate(data.table(SQM$orfs$abund), by=list(funs), FUN=sum)
    rownames(abund)            = abund[,1]
    abund                      = as.matrix(abund[,-1,drop=F])

    bases                      = aggregate(data.table(SQM$orfs$bases), by=list(funs), FUN=sum)
    rownames(bases)            = bases[,1]
    bases                      = as.matrix(bases[,-1,drop=F])

    coverage                   = aggregate(data.table(SQM$orfs$cov)  , by=list(funs), FUN=sum)
    ...
    if(is.null(dim(lengths)))  { lengths = data.table(matrix(lengths, nrow=1)) } # The replicate function generates a vector if there is only one ORF. Avoid that.
    ...
    copies                     = aggregate(data.table(SQM$orfs$abund>0), by=list(funs), FUN=sum) # aggregate 1 if present, 0 if absent
    rownames(copies)           = copies[,1]
    copies                     = as.matrix(copies[,-1,drop=F])

    tpm                        = aggregate(data.table(SQM$orfs$tpm)  , by=list(funs), FUN=sum)
    rownames(tpm)              = tpm[,1]
