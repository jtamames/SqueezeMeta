...
#' }
#' @param project_path character, project directory generated by SqueezeMeta, or zip file generated by \code{sqm2zip.py}.
...
#' @export
loadSQM = function(project_path, tax_mode = 'prokfilter', trusted_functions_only = FALSE, load_sequences = TRUE, engine = 'data.table')
    {
...
loadSQM_ = function(project_path, tax_mode = 'prokfilter', trusted_functions_only = FALSE, load_sequences = TRUE, engine = 'data.table')
    ...
    ### COPY NUMBERS
    if(file.exists.zip(project_path, sprintf('results/tables/%s.RecA.tsv', project_name)))
        {
        ### KEGG
        if(has_KEGG)
            {
