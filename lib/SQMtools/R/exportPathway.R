...
#' @param log_scale logical. Use a base 10 logarithmic transformation for the color scale (default \code{FALSE}).
#' @param max_scale_value numeric. Maximum value to include in the color scale. By default it is the maximum value in the selected samples (default \code{NULL}).
...
#' @export
exportPathway = function(SQM, pathway_id, count = 'tpm', samples = NULL, split_samples = F, sample_colors = NULL, log_scale = F, max_scale_value = NULL, color_bins = 10, output_suffix = 'pathview')
    {
    ### Check params.
    ...
    breaks = seq(min(submat), max_scale_value, length.out=color_bins)
    submat_color = matrix(findInterval(submat, breaks), nrow=nrow(submat), dimnames=dimnames(submat)) # The lowest index in the interval is 1. This contains zeros as well as low values.
    ...
    # Customize colors for each sample and create legends.
    bg.col = "#FFFFFF"
	...
	# Print legend
	if(!log_scale)    { true_breaks = breaks } else { true_breaks = 10 ** breaks; true_breaks[1] = 0 }
	filename = sprintf('ko%s.%s.%s.legend.png', pathway_id, output_suffix, colnames(mat)[i])
