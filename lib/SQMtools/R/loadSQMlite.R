...
#' A SQMlite object can be used for plotting and exporting, but it can not be subsetted or combined.
#'
...
#' If the original project was created with sqm_longreads.pl, the number of counts in the taxonomy and functional tables will not be the same: each count in the taxonomy tables will refer to an individual read, while each count in the functional tables will refer to an individual ORF within the reads. SQMlite objects generated from sqm_longreads results will have an additional attribute named \code{total_orfs}.
