...
USAGE: anvi-load-sqm.py [-h] -p PROJECT -o OUTPUT [--num-threads NUM_THREADS]
       ...
       --run-scg-taxonomy                     Just for anvio 6. Perform taxonomic assignation of single-copy core genes based on The Genome Taxonomy Database (GTDB) Parks, D. et al. (2018) doi:10.1038/nbt.4229. Note that if it is the first time you use it in anvio 6, it would be necessary to run the command anvi-setup-scg-databases to set up the database (more info: http://merenlab.org/2019/10/08/anvio-scg-taxonomy/)
...
ABOUT ANVI'O:
    ...
    - If you want to explore ways to manually create the database, and other cool stuff that you can do with anviâo
...
def main(args):
    ...
    if args.run_scg_taxonomy and version >= 6:
        #command = ['anvi-setup-scg-databases', '-T', '4'] # ONLY NEEDED THE FIRST TIME
        #run_command(command)
        if not args.run_HMMS:
            print('Running HMMs first (It is necessary to run run_scg_taxonomy after)')
            command = ['anvi-run-hmms', '-c', '{}/CONTIGS.db'.format(args.output), '--num-threads' , args.num_threads]
            run_command(command)
        command = ['anvi-run-scg-taxonomy', '-c', '{}/CONTIGS.db'.format(args.output), '--num-parallel-processes', '3', '--num-threads', args.num_threads]
        run_command(command)
    elif version < 6:
...
#! THIRD: Make profiles.db. Load bam files, important: distinguish number of samples
    ...
    if not samples:
        ...
        command = ['anvi-merge', '{}/PROFILE.db'.format(profile_name),  '{}/PROFILE.db'.format(profile_name),'-c','{}/CONTIGS.db'.format(args.output),'-o','{}/temp'.format(args.output),'--skip-hierarchical-clustering']
        if version < 6:# NEW
        ...
        command = ['anvi-merge'] + profile_dir + ['-c','{}/CONTIGS.db'.format(args.output),'-o','{}/temp'.format(args.output),'--skip-hierarchical-clustering'] ## ADD SAMPLE-NAME??
        if version < 6:# NEW
            command.append('--skip-concoct-binning') #NEW
        run_command(command)
