...
###/home/tamames/software/miniconda3/envs/SqueezeMeta1.8/SqueezeMeta/utils/
...
my($numthreads,$aadir,$project,$samplesfile,$notax,$nocog,$nokegg,$nopfam,$gtdb,$blastmode,$blocksize,$hel,$blockoption,$printversion,$opt_db,$databasepath,$force_overwrite);

my $start_run = time();
print BOLD "\nRedPepper_annot v$version - (c) J. Tamames, F. Puente-Sánchez CNB-CSIC, Madrid, SPAIN\n\nThis is part of the SqueezeMeta distribution (https://github.com/jtamames/SqueezeMeta)\nPlease cite: Tamames & Puente-Sanchez, Frontiers in Microbiology 10.3389 (2019). doi: https://doi.org/10.3389/fmicb.2018.03349\n\n"; print RESET;

my $helpshort="Usage: SQM_annot.pl -f <faa directory> -s <samples file> [options]\n";

my $helptext = <<END_MESSAGE;
Usage: SQM_annot.pl -f <files directory> -s <samples file> [options]
...
Mandatory parameters:
 ...
 Options:
   ...
   --gtdb: Taxonomic annotation using GTDB-tk
...
if($force_overwrite) { $command.=" --force_overwrite"; }
...
foreach my $thissample(keys %files) {
	$numsample++;
	print "Working with sample $numsample: $thissample\n";
	my $aafile=$files{$thissample};
	if($aafile=~/\.gz$/) { system("gunzip $aadir/$aafile"); $aafile=~s/\.gz$//; }
	$project=$thissample;
...
my $command="mkdir SQM/mag_summary; mkdir SQM/mag_aafiles;";
if(-e "SQM/mag_summary") {} else { system($command); }
my $command="mv SQM/$project/results/*summary SQM/mag_summary/; mv SQM/$project/results/*faa SQM/mag_aafiles/; mv SQM/$project/results/*fna mag_aafiles/;";
system($command);

#my $command="rm -r SQM/$project";
#system($command);
	}
...
sub funclass {
	...
	my %fils;
	if(!$nocog) { %fils=("COG","SQM/$project/results/07.$project.fun3.cog"); }
	if(!$nokegg) { %fils=("KEGG","SQM/$project/results/07.$project.fun3.kegg"); }
	...
	foreach my $tcase(sort keys %fils) {
		my $outfile="SQM/$project/results/$project.$tcase.summary";
	...
	open(in,$opt_db) || die "Cannot open external database $opt_db\n";
	while(<in>) {
		chomp;
		next if !$_;
		...
		close infile2;
			...
			close inc;
			my $outfile="SQM/$project/results/$project.$db.summary";
			open(out,">$outfile") || die "Cannot open output file $outfile\n";
