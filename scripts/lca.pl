...
my($thisorf,$lastorf,$validhits,$tothits,$refscore,$refiden);
...
sub query {
	my($refcc,$genocc,$unicc,$verbose,$ratioscore,$idendiff)=0; 
	...
	if($refcc) {
		...
		if($list[8]) { $validhits++;  }			#-- Count the number of valid hits
		}
	}
