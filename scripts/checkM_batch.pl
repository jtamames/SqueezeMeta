...
our($datapath,$taxlist,%bindirs,$checkm_soft,$alllog,$resultpath,$tempdir,$numthreads);
...
foreach my $m(@files) { 
	...
	while($inloop) { 
		...
		if(-e $marker) {} else { 
			my $command="$checkm_soft taxon_set $rank $tax $marker > /dev/null";
			system($command); 
			}
		...
		system("$checkm_soft analyze -t $numthreads -x $fastafile $marker $bindir $checktemp > /dev/null");
		system("$checkm_soft qa -t $numthreads $marker $checktemp >> $checkmfile");
